...
> ⚠️ 此區域已精簡。歷史 Dev Check Log 全部移除（30+ 天重複測試日誌無助開發）。
> 問題狀態追蹤請參考 Known Issues（K01-K19）。

**最近檢查：2026-05-22**
- 後端運行正常（port 3000）
- DB: 26 patients, 19 appointments
- K01-K19: 所有已知問題狀態不變，無新問題發現

**Dev Check Log 完整歷史**（如需查閱）：請查看 Git commit history 或早期版本。
...
**ISAF CSV 欄位結構（26 欄，見 CSV header）：**
`mednbr, product_name_zh, product_name_pt, product_name_en, pharmaceutical_form_zh, pharmaceutical_form_pt, pharmaceutical_form_en, route_of_administration_zh, route_of_administration_pt, route_of_administration_en, active_ingredients_zh, active_ingredients_pt, active_ingredients_en, legal_classification_zh, legal_classification_pt, legal_classification_en, atc_classification_zh, atc_classification_pt, atc_classification_en, manufacturer_zh, manufacturer_pt, manufacturer_en, distributor_code, distributor_zh, distributor_pt, distributor_en`
